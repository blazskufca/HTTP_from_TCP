...
class TestHeadersParse(unittest.TestCase):
    ...
    def test_valid_two_headers_with_existing_headers(self):
        headers = Headers()
    ...
    def test_valid_done(self):
        ...
        n, done = headers.parse(data)
        self.assertEqual(len(headers.headers), 0)
    ...
    def test_valid_uppercase_single_header(self):
        headers = Headers()
        data = b"HOST: localhost:42069\r\n\r\n"
        n, done = headers.parse(data)
        self.assertEqual(headers["host"], "localhost:42069")
    ...
    def test_invalid_char_in_header_key(self):
        headers = Headers()
        data = b"H@st: localhost:42069\r\n\r\n"
        with self.assertRaises(ValueError):
            headers.parse(data)

    def test_valid_uppercase_with_existing_header(self):
        headers = Headers()
